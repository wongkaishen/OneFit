...
export default function GymProgressPage() {
  ...
  // Copy a plain-text progress summary to the clipboard.
  const share = async () => {
    const latest = (progress.data ?? [])[0];
    if (!latest) return;
    const text = `My OneFit progress — weight ${latest.weight ?? "—"}kg, body fat ${latest.body_fat_percent ?? "—"}%`;
    try {
            ...
            <div>
              ...
              {(milestones.data ?? []).map((m) => (
                <div key={m.milestone_id}>
                  <div className="flex items-center justify-between py-3">
                    <div>
                      <div className="font-sans text-[14px] text-charcoal">{m.title ?? "Milestone"}</div>
